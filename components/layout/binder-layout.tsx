...
export function BinderLayout({ children }: { children: React.ReactNode }) {
  ...
  const emailVerified = (session?.user as any)?.emailVerified as boolean | undefined;
  const showExamTrackModal = session?.user && !examTrack;
  const showEmailBanner = session?.user && emailVerified === false && !bannerDismissed;
  ...
  const handleResendVerification = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        toast.error(data.error || 'Gönderilemedi. Lütfen tekrar deneyin.');
      }
    } catch {
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-hidden relative z-10">
        ...
        <AnimatePresence>
          {showEmailBanner && (
            ...
            >
              <div className="mt-4 flex items-center gap-3 rounded-2xl bg-amber-500/10 border border-amber-500/20 px-4 py-3 text-sm">
                <AlertTriangle className="w-4 h-4 text-amber-400 flex-shrink-0" />
                <p className="text-amber-200/80 flex-1">
                  E-posta adresiniz doğrulanmadı.
                </p>
                {resendSuccess ? (
                  <span className="flex items-center gap-1.5 text-emerald-400 text-xs font-medium flex-shrink-0">
                  ...
                  <button
                    ...
                    disabled={resending}
                    className="flex items-center gap-1.5 text-amber-300 hover:text-amber-200 text-xs font-semibold transition-colors disabled:opacity-50 flex-shrink-0"
                  >
                    {resending ? (
