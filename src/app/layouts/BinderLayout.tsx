...
import { TEXTURES } from '../components/ui/Skeuomorphic';
import { LayoutDashboard, CheckSquare, GraduationCap, BarChart2, Image as ImageIcon, UserCircle } from 'lucide-react';
import { motion } from 'motion/react';

export const BinderLayout = () => {
    ...
    >
      ...
      <div className="relative w-full max-w-6xl h-[85vh] flex rounded-3xl overflow-hidden shadow-2xl bg-slate-900 border-8 border-slate-800 ring-1 ring-white/10">
        ...
        >
          ...
          <div className="p-6 pt-10 flex flex-col items-center gap-3 border-b border-white/10 bg-black/20 backdrop-blur-sm">
            ...
            <div className="text-center">
              <h2 className="text-yellow-50 font-serif text-lg tracking-wide">{currentUser === 'seyda' ? 'Şeyda A.' : 'Furkan K.'}</h2>
              <button 
                onClick={() => switchUser(currentUser === 'seyda' ? 'furkan' : 'seyda')}
                className="text-yellow-500 text-xs hover:text-yellow-300 transition-colors uppercase tracking-widest font-bold mt-1"
              >
                Kullanıcı Değiştir
              </button>
            </div>
        ...
        <div className="flex-1 bg-slate-100 relative overflow-hidden flex flex-col">
           ...
           {/* Content Render */}
           <div className="flex-1 overflow-y-auto p-4 sm:p-8 md:p-12 relative z-0">
             <motion.div
